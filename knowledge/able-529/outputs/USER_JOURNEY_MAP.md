...
| 3. Select funding path | Choose direct rollover, indirect rollover, or contribution | Poor understanding of path-specific requirements and destination-plan tax variance | Wrong path, timing/document errors, destination choice with avoidable recapture risk | Path explainer + destination-aware state warning |
| 4. Pre-transfer compliance | Validate docs and relationship/cap checks | Missing source-plan earnings/principal docs; annual-limit headroom uncertainty | Submission blocked, misclassified, or rejected for limit overrun | Hard-stop preflight validator + cap headroom engine |
...
3. Destination plan selected without state-specific recapture logic.
