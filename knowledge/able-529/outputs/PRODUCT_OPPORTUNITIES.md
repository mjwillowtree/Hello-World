...
| 4 | **Authority-conflict sentinel + escalation routing** | Teams over-trust a single source when forms and bulletins diverge | Indiana IN-CR vs Bulletin #98 Tier-1 conflict shows live drift risk | 5 | 3 | Must preserve source precedence and show unresolved conflicts | High |
| 5 | **Transfer ops tracker (SLA + exception queue)** | Users abandon when transfer status is opaque | Multi-step manual workflow across institutions; Invest529->ABLEnow two-system handoff | 4 | 3 | Requires auditable event trail | Medium-High |
...
- Source-routing friction (which institution gets which transfer form) is now confirmed in official Invest529 guidance.
- Tier-1 source conflicts can exist within a state; product needs conflict-state handling instead of binary "known/unknown" labels.
