...
| 4 | **Contribution-cap reservation + overflow guardrail** | Transfers fail or get rejected when annual ABLE limit is exceeded mid-process | STABLE direct rollover form warns over-limit contributions are rejected in entirety | 5 | 3 | Must track all contributor inflows and preserve auditability of cap decisions | High |
...
- Indiana evidence shows state treatment can vary by **destination ABLE plan type** (in-state vs other), strengthening need for destination-aware rules.
- STABLE direct rollover operations elevate contribution-cap rejection controls from optional to ship-critical.
