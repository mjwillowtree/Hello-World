...
5. **Annual-limit reservation ledger (pre-submit hard check)**
   - Reserve ABLE contribution headroom when transfer is initiated, not only when funds post.
   - Block/route transfers where reserved + posted inflows exceed annual cap.
   - Capture rationale and evidence for each reservation/release event.
...
3. **Cap-reservation checkpoint experiment**
   - Metric: over-limit rejections, time-to-fund, manual review volume.
