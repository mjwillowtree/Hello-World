...
5. **Provider transition/blackout policy service**
   - Maintain a dated calendar of provider blackout windows, servicing transitions, and temporary form outages.
   - Prevent or warn on actions likely to fail during blackout periods (new enrollments, mailed contributions, withdrawal timing).
   - Attach provider-specific messaging and remediation guidance to affected workflows.
...
3. **Blackout-window intervention test**
   - Metric: failed transfer attempts during known blackout windows, support contacts per transfer, delay-driven abandonment.
