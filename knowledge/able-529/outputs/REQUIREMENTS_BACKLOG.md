...
5. **Signature-threshold rules check (medallion/notary)**
   - Evaluate plan-specific signature constraints before packet completion.
   - Example rule: trigger medallion path when transfer amount exceeds threshold and signer role mismatch exists.
   - Provide branch output: "can complete digitally", "needs medallion", or "needs source-plan notary."
...
3. **Signature-preclear interstitial**
   - Metric: pre-submit fallout vs post-submit rejection rate for medallion/notary-triggered cases.
