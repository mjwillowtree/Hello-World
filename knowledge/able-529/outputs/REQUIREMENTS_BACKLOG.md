...
5. **Source-plan requirement discovery service (new)**
   - Before packet generation, require source-plan policy check:
     - whether Medallion Signature Guarantee is required
     - whether source plan requires wet signatures, notarization, or specific transfer forms
     - whether principal/earnings certification is mandatory for correct posting
   - Persist evidence URL and retrieval date to decision record; block "ready to submit" if unresolved.
...
3. **Source-plan guarantee precheck experiment**
   - Metric: late-stage rejection rate due to missing guarantee; median days from packet creation to accepted transfer.
