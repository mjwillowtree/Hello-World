...
2. **State treatment remains the biggest implementation risk.**
   ...
   - Utah now has line-level addback mapping (TC-40A code 54 -> TC-40 line 5), but explicit 529->ABLE carveout text is still unresolved.
...
4. **Contribution-cap timing is a separate failure mode.**
   - Official rollover forms indicate 529 rollover amounts count toward standard annual ABLE limits, so in-flight transfers can fail if headroom is not reserved.
...
4. In-flight annual-cap overrun caused by concurrent contributions during rollover processing.
...
3. Contribution-cap reservation control for in-flight rollover protection.
