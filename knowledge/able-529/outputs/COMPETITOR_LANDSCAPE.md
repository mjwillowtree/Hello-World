...
| PA ABLE | Incoming direct rollover process includes conditional Medallion Signature Guarantee and source-plan certification requirements | Missing guarantee can delay transfer; missing principal/earnings certification can force full amount to be treated as earnings pending statement | Official incoming direct rollover form | High |
...
### PA ABLE
- Form instructs users to check whether source plan requires a signature guarantee; if required and omitted, rollover can be delayed.
- Form states PA ABLE will treat the entire transferred amount as earnings until source plan provides contribution/earnings breakdown certification.
...
5. Source-plan-specific guarantee requirements must be discovered before packet generation, not at final submission.
