...
| PA ABLE | Incoming direct rollover form covers ABLE->ABLE and 529->ABLE with explicit rejection/guarantee conditions | Source-plan signature-guarantee dependencies and missing basis statements can delay/reclassify transfers | Official incoming direct rollover form | High |
...
### PA ABLE
- Incoming direct rollover form explicitly states 529 rollover contributions that exceed annual or balance limits will be rejected.
- Form warns signature guarantees may be required by the current plan and that missing contribution/earnings breakdown can cause the plan to treat the full transferred amount as earnings until documentation arrives.
...
5. Source-plan dependency checks (medallion/signature guarantee + basis statement) should be preflighted before packet generation.
