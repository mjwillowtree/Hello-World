...
| PA ABLE | Incoming direct rollover workflow includes contingent Medallion Signature Guarantee and cap-driven rejection controls | Signature-guarantee surprises and cap overflow checks create late-stage failures if not preflighted | Official PA ABLE incoming direct rollover form | High |
...
### PA ABLE
- Form states that if the source plan requires a signature guarantee, Section 5 must be completed (Medallion workflow dependency).
- Form states that absent contribution/earnings certification from source program manager, the full contribution is treated as earnings until corrected.
- Form states rollover contributions exceeding annual contribution or account-balance limits will be rejected.
...
5. Rollover limit-overflow rejection rules should be validated before packet generation.
