...
| State tax conformity + recapture/add-back (50-state) | 3 | 3 | High | Indiana and NJ are now line-mapped, but PA and Utah remain priority closures; Indiana hierarchy conflict requires tax-year-safe implementation rule. |
| Program-level operational constraints (plan forms, timing) | 3 | 3 | Med | Official forms now include explicit dollar-threshold medallion friction evidence (Fidelity); still need transfer SLA distribution data. |
...
2. Resolve Indiana IN-CR vs Bulletin #98 conflict into implementation-safe state rule logic.
