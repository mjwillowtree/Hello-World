...
| State tax conformity + recapture/add-back (50-state) | 3 | 3 | High | Matrix now has 12 substantive rows with Indiana and Utah advanced; NJ/PA line-level closure remains highest priority. |
| Program-level operational constraints (plan forms, timing) | 3 | 3 | Med | ABLE United adds deadline/notarization controls; still need empirical SLA timing distributions (p50/p90). |
...
2. Pull Indiana Schedule 4 exact line-level recapture entry and lock to High+ confidence.
...
6. "Destination ABLE choice does not affect state recapture." (false in Indiana guidance)
