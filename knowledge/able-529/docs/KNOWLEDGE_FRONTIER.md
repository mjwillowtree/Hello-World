...
| State tax conformity + recapture/add-back (50-state) | 3 | 2 | High | Matrix scaffold spans 51 jurisdictions with 11 substantive rows; NJ/PA line-level closure remains highest priority, and Indiana now has filing-year conflict requiring form-refresh validation. |
| Program-level operational constraints (plan forms, timing) | 3 | 3 | Med | Official forms from Fidelity, ABLE for ALL, Texas ABLE, and PA ABLE confirm documentation/signer failure modes; still need SLA timing evidence. |
...
2. Advance Utah to first-pass Tier-1 mapping (code + form lines).
3. Expand operations lane from form requirements to timing/SLA evidence and exception-frequency measurement.
