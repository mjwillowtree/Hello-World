...
| Federal 529->ABLE rollover legality and limits | 4 | 4 | Med | Sunset-removal position now strong; remaining work is implementation artifact reconciliation (forms/publications lag). |
...
| State tax conformity + recapture/add-back (50-state) | 2 | 2 | High | Expanded to 9-state watchlist, but still missing line-cited final positions for most jurisdictions. |
...
1. Build a line-cited 12-state recapture matrix (form line + conformity year + authority tier).
2. Build adversarial comparison sheet: 529 distribution vs 529->ABLE vs 529->Roth IRA.
3. Resolve ABLE-to-Work retirement-plan participation edge mechanics with examples.

## Assumptions to challenge every run

1. "Federal conformity means state conformity." (tested again this run; still false in practice)
...
4. "If IRS newsroom says it, implementation details are settled." (explicitly challenged; older newsroom sunset language can lag statute)
